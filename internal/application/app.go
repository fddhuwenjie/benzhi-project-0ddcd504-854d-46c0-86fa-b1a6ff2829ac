...
type App struct {
	...
	locks                  sync.Map
	createMu               sync.Mutex
	manifestVerificationMu sync.Mutex
	manifestVerifications  map[string]manifestVerificationEntry
}

type manifestVerificationEntry struct {
	manifest     *domain.PreservationManifest
	head         string
	caseDigest   string
	verification domain.ManifestVerification
	reasons      []string
}

func New(s *store.Store, a *audit.Audit) *App {
	return &App{Store: s, Audit: a, manifestVerifications: map[string]manifestVerificationEntry{}}
}
...
func (a *App) ManifestVerificationDetails(id string) (*domain.PreservationManifest, string, domain.ManifestVerification, []string, error) {
	...
	head := a.Audit.Head(id)
	caseDigest, err := digest(c)
	if err != nil {
		return nil, "", domain.ManifestVerification{}, nil, err
	}
	a.manifestVerificationMu.Lock()
	if cached, ok := a.manifestVerifications[id]; ok && cached.caseDigest == caseDigest && cached.head == head {
		a.manifestVerificationMu.Unlock()
		return cached.manifest, cached.head, cached.verification, cached.reasons, nil
	}
	a.manifestVerificationMu.Unlock()
	...
	a.manifestVerificationMu.Lock()
	a.manifestVerifications[id] = manifestVerificationEntry{manifest: c.Manifest, head: head, caseDigest: caseDigest, verification: verification, reasons: reasons}
	a.manifestVerificationMu.Unlock()
	return c.Manifest, head, verification, reasons, nil
}
