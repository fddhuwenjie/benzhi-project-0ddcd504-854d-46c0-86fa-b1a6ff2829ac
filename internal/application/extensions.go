...
type custodyChainCacheKey struct {
	caseID        string
	fromTime      string
	toTime        string
	limit         int
...
func (a *App) CustodyChain(id string, fromTime, toTime *time.Time, limit int, includeEvents bool) (domain.CustodyChainResult, error) {
	...
	if toTime != nil {
		key.toTime = toTime.UTC().Format(time.RFC3339Nano)
	}
	a.custodyMu.Lock()
	if cached, ok := a.custodyChains[key]; ok {
		a.custodyMu.Unlock()
		return cloneCustodyChainResult(cached), nil
	}
	a.custodyMu.Unlock()
	...
	a.custodyMu.Lock()
	a.custodyChains[key] = cloneCustodyChainResult(result)
	a.custodyMu.Unlock()
	return result, nil
}

func cloneCustodyChainResult(source domain.CustodyChainResult) domain.CustodyChainResult {
	cloned := source
