...
func (a *App) cachedBatchResult(key, requestDigest string) (domain.RegistrationBatchResult, bool, error) {
	a.batchMu.Lock()
	defer a.batchMu.Unlock()
	entry, ok := a.batchResults[key]
	if !ok {
		return domain.RegistrationBatchResult{}, false, nil
	}
	if entry.RequestDigest != requestDigest {
		return domain.RegistrationBatchResult{}, false, domain.ErrConflict
	}
	return entry.Result, true, nil
}

func (a *App) rememberBatchResult(key, requestDigest string, result domain.RegistrationBatchResult) {
	a.batchMu.Lock()
	defer a.batchMu.Unlock()
	a.batchResults[key] = batchIdempotencyEntry{RequestDigest: requestDigest, Result: result}
}
...
func (a *App) CreateBatch(requestID, mode string, items []domain.RegistrationItem) (domain.RegistrationBatchResult, error) {
	...
	if cached, ok, cacheErr := a.cachedBatchResult(key, requestDigest); ok || cacheErr != nil {
	...
	if saved, ok := a.Store.GetIdempotency(key); ok {
		...
		a.rememberBatchResult(key, requestDigest, entry.Result)
	...
	a.rememberBatchResult(key, requestDigest, result)
	return result, nil
}
