...
// AuditQueryError 为审计层返回的故障补充个案上下文。
type AuditQueryError struct {
	CaseID    string
	CauseText string
}

func (e *AuditQueryError) Error() string {
	return fmt.Sprintf("个案 %s 的审计查询失败: %s", e.CaseID, e.CauseText)
}

func auditQueryError(caseID string, cause error) error {
	return &AuditQueryError{CaseID: caseID, CauseText: cause.Error()}
}
...
func (a *App) AuditSearch(id string, query domain.AuditQuery) (domain.AuditPage, error) {
	...
	if err != nil {
		return domain.AuditPage{}, auditQueryError(id, err)
	}
