...
func (s *Server) handleAssessmentHistory(w http.ResponseWriter, r *http.Request, id string) {
	...
	s.assessmentHistoryMu.RLock()
	hist, cached := s.assessmentHistories[id]
	if cached {
		hist = append([]domain.ConditionAssessment(nil), hist...)
	}
	s.assessmentHistoryMu.RUnlock()
	if !cached {
		hist = append([]domain.ConditionAssessment(nil), c.AssessmentHistory...)
		if c.Assessment != nil {
			hist = append(hist, *c.Assessment)
		}
		s.assessmentHistoryMu.Lock()
		s.assessmentHistories[id] = append([]domain.ConditionAssessment(nil), hist...)
		s.assessmentHistoryMu.Unlock()
	}
...
func (s *Server) handleRecaptureTimeline(w http.ResponseWriter, r *http.Request, id string) {
	...
	out(w, map[string]interface{}{"case_id": id, "timeline": rows, "items": rows, "remaining_attempts": remaining, "escalation": c.CurrentEscalationRequirement, "audit_head": s.App.Audit.Head(id), "revision": c.Revision}, 200)
}
