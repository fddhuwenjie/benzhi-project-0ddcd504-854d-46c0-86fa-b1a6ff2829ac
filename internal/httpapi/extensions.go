...
func (s *Server) handleAuditSearch(w http.ResponseWriter, r *http.Request, id string) {
	...
	result := make(chan auditSearchResult, 1)
	go func() {
		page, searchErr := s.App.AuditSearch(id, query)
		result <- auditSearchResult{page: page, err: searchErr}
	}()
	select {
	case <-r.Context().Done():
		out(w, map[string]string{"error": "审计查询已取消"}, 499)
	case search := <-result:
		if search.err != nil {
			parseErr(w, search.err)
			return
		}
		out(w, search.page, http.StatusOK)
	}
}
