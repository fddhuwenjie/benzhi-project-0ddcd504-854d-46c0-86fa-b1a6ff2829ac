...
type Audit struct {
	...
	headAtCache headAtCacheEntry
}

type headAtCacheEntry struct {
	caseID string
	count  int
	digest string
}
...
func (a *Audit) HeadAt(id string, count int) string {
	if cached := a.headAtCache; cached.caseID == id && cached.count == count {
	...
	events := append([]domain.Event(nil), a.events[id][:count]...)
	a.mu.Unlock()
	head := ""
	...
	a.headAtCache = headAtCacheEntry{caseID: id, count: count, digest: head}
	return head
}
