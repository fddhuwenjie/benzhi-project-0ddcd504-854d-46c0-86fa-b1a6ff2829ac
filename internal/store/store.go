...
type Store struct {
	...
	accessionCache map[string]*domain.DigitizationCase
}
...
func New(dir string) (*Store, error) {
	...
	s := &Store{dir: dir, cases: map[string]*domain.DigitizationCase{}, idem: map[string][]byte{}, accessionCache: map[string]*domain.DigitizationCase{}}
...
func (s *Store) FindByAccession(accession string) *domain.DigitizationCase {
	s.mu.Lock()
	defer s.mu.Unlock()
	if cached, ok := s.accessionCache[accession]; ok {
		return cloneCase(cached)
	}
	for _, c := range s.cases {
		if c.AccessionCode == accession {
			cached := cloneCase(c)
			s.accessionCache[accession] = cached
			return cloneCase(cached)
		}
	}
	return nil
