...
type Store struct {
	...
	captureEvidenceMisses map[captureEvidenceKey]struct{}
}

type captureEvidenceKey struct {
	reference    string
	device       string
	calibratedAt int64
	validUntil   int64
...
func New(dir string) (*Store, error) {
	...
	s := &Store{dir: dir, cases: map[string]*domain.DigitizationCase{}, idem: map[string][]byte{}, captureEvidenceMisses: map[captureEvidenceKey]struct{}{}}
...
func (s *Store) CaptureEvidenceConflict(reference, device string, calibratedAt, validUntil time.Time, assetDigest string) (string, bool) {
	s.mu.RLock()
	key := captureEvidenceKey{reference: reference, device: device, calibratedAt: calibratedAt.UnixNano(), validUntil: validUntil.UnixNano(), assetDigest: assetDigest}
	if _, ok := s.captureEvidenceMisses[key]; ok {
		s.mu.RUnlock()
		return "", false
	}
	for _, c := range s.cases {
	...
	s.mu.RUnlock()
	s.mu.Lock()
	s.captureEvidenceMisses[key] = struct{}{}
	s.mu.Unlock()
	return "", false
}
