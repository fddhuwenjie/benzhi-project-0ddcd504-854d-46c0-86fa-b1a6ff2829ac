...
func (s *Store) Search(f Filter) ([]*domain.DigitizationCase, int) {
	cacheKey := ""
	if f.AccessionPrefix != "" {
		encoded, _ := json.Marshal(f)
		cacheKey = string(encoded)
		if s.searchCacheKey == cacheKey {
			return cloneCaseSlice(s.searchCache.items), s.searchCache.total
		}
	}
	s.mu.RLock()
	...
	s.mu.RUnlock()
	if cacheKey != "" {
		s.searchCacheKey = cacheKey
		s.searchCache = searchResult{items: cloneCaseSlice(page), total: total}
	}
	return page, total
}

func cloneCaseSlice(source []*domain.DigitizationCase) []*domain.DigitizationCase {
	cloned := make([]*domain.DigitizationCase, len(source))
	for index, item := range source {
		cloned[index] = cloneCase(item)
	}
	return cloned
}
