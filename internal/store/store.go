...
type Store struct {
	...
	resourceConflicts map[string][]ResourceConflict
}
...
func New(dir string) (*Store, error) {
	...
	s := &Store{dir: dir, cases: map[string]*domain.DigitizationCase{}, idem: map[string][]byte{}, resourceConflicts: map[string][]ResourceConflict{}}
...
func (s *Store) PlanResourceConflicts(caseID string, plan domain.CapturePlan) []ResourceConflict {
	...
	cacheKey := strings.Join([]string{
		caseID,
		strings.ToUpper(strings.TrimSpace(plan.PlaybackDevice)),
		strings.ToUpper(strings.TrimSpace(plan.Operator)),
		plan.ScheduledStart.UTC().Format(time.RFC3339Nano),
		plan.ScheduledEnd.UTC().Format(time.RFC3339Nano),
	}, "\x00")
	s.mu.Lock()
	defer s.mu.Unlock()
	if cached, ok := s.resourceConflicts[cacheKey]; ok {
		return append([]ResourceConflict(nil), cached...)
	}
	now := time.Now().UTC()
	...
	s.resourceConflicts[cacheKey] = append([]ResourceConflict(nil), conflicts...)
	return append([]ResourceConflict(nil), conflicts...)
}
